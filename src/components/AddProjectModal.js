...
const AddProjectModal = ({ isModalOpen, closeModal, edit = false, id = null }) => {
    ...
    useEffect(() => {
        if (edit && isModalOpen) {
            axios.get(`${process.env.REACT_APP_API_URL}/project/${id}`)
                .then((res) => {
    ...
    const handleSubmit = (e) => {
        e.preventDefault()
        const request = edit 
            ? axios.put(`${process.env.REACT_APP_API_URL}/project/${id}`, { title, description: desc })
            : axios.post(`${process.env.REACT_APP_API_URL}/project/`, { title, description: desc });
